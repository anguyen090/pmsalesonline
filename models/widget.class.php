...
class Widgets extends Database {
    ...
    public function getWidget_update_up(){
    ...
    */
    ...
    public function getARecord($id){
        ...
        $condition = array(
            'where'     => 'widget_status = 1 AND widget_id = '.$id,
        );
    ...
    private function checkState($widgetType,$type,$state,$num){
        ...
        //Kiểm tra xem có tồn tại biến type hay không?
        $typeWhere = isset($type) &&  intval($type)>0 ? ' AND loai'.$widgetType.'_id='.$type : "";
        ...
        if($state == 'new'){
            $stateWhere = array(
                'where'         => $widgetType.'_status=1'.$typeWhere,
				'order_by'		=> $widgetType.'_order DESC, '. $widgetType.'_update_date DESC',
                'limit_start'   => '0',
                'limit_end'     => $num
            );
        }
        //Nếu là bài viết hoặc sản phẩm được xem nhiều
        if($state == 'view'){
            $stateWhere = array(
                'where'         => $widgetType.'_status=1',
				'order_by'		=> $widgetType.'_view DESC, '. $widgetType.'_update_date DESC',
                'limit_start'   => '0',
                'limit_end'     => $num
            );
        }
    ...
    public function checkWidgetContent($widgetID){
        ...
        if($widgetID == 0){
            ...
            {
                $style = '';
				$style = str_replace('<p>','',htmlspecialchars_decode($widget->widget_style));
				$style = str_replace('</p>','',htmlspecialchars_decode($style));
				$widgetContent['widgetStyle'] = $style!="" || $style!=0 ? $style : "";
                ...
                if($widgetType == 'quangcao'){
				   ...
				   $widgetContent['widgetStyle'] = $widget->widget_style!="" || $widget->widget_style!=0 ? htmlspecialchars_decode($widget->widget_style) : "";
                   if(in_array("title",$view_list,true)){$widgetContent['widgetTitle'] = $widget->widget_title;}
                   ...
                   if(in_array('content', $view_list,true)) {$widgetContent['widgetContent'] = htmlspecialchars_decode(str_replace('../',MTN_BASE_SITEURL.'/',$widget->widget_content));}
                    ...
                    $getContent = $this->checkState($widgetType,$type,$state,$num);
                    $i=0;
                    ...
                    foreach($getContent as $getContent)
                    {
                        ...
                        if(in_array("0",$view_list,true)){
                            ...
                            if($i==1){
                                $image = '<img src="'.str_replace("../",MTN_BASE_SITEURL.'/',str_replace("noidung/","noidung/",$getContent->noidung_images)).'" alt="'.$getContent->noidung_title.'" />';
                                $titleIcon = false;
                            }
                        }
                        ...
                        $widgetContent[$i]['link'] = MTN_BASE_SITEURL.MTN_URLRewrite.$lnd->RootContentAliasLink($getContent->loainoidung_id,"").$getContent->noidung_id.'-'.$f->convertViToEn(stripTags(str_replace("../",MTN_BASE_SITEURL.'/',htmlspecialchars_decode($getContent->noidung_title)),false)).'.html';
                        ...
                        $widgetContent[$i]['view'] = $getContent->noidung_view;
                        $widgetContent[$i]['comment'] = $getContent->noidung_count_command;
                        ...
                        if (in_array('update', $view_list, true))		{$widgetContent[$i]['updateDate'] = $getContent->noidung_update_date;}
                        ...
                        if(in_array('contentby',$view_list,true))       {$widgetContent[$i]['contentby'] = $user->getFullname($getContent->user_id); }
                        //Nếu cho phép hiển thị loại nội dung
        ...
        if(isset($widgetContent) && !empty($widgetContent)){
