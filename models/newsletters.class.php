...
class Newsletter extends Database {
    ...
    //$newsletters_isread
    public function setNewsletters_isread($newsletters_isread){
        $this->newsletters_isread = $newsletters_isread;
    }
    public function getNewsletters_isread(){
        return $this->newsletters_isread;
    }
    //$newsletters_status
    ...
    public function getNewslettersByCustom($email,$option,$parent,$value){
        $db = new Database();
        $condition = array(
    ...
    /**
     * Hàm lấy email đăng ký mới nhất (Email chưa đọc) để thông báo cho admin biết có người đăng ký
     * $type: Giá trị cần lấy: 1 hoặc 0
     * $value: Các trường giá trị cần lấy
     */
    public function countNewsLettersRegister($type,$value){
        $db = new Database();
        $condition = array(
            'where'     => 'newsletters_isread='.intval($type)
        );
        $result = $db->countData('newsletters',$value,$condition);
        return $result;
    }
}